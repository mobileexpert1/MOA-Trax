...
public class PDFView extends RelativeLayout {
    ...
    Float scale;
    float  curLatitude = 0.0F;
    float curLongitude = 0.0F;
  //  private VmTiff vmTiff;
    /**
    ...
    public boolean canScrollHorizontally(int direction) {
        ...
        Log.d("TAG", "Zero" + direction);
        if (swipeVertical) {
            if (direction < 0 && currentXOffset < 0) {
                Log.d("TAG", "One" + currentXOffset);
                return true;
            } else if (direction > 0 && currentXOffset + toCurrentScale(pdfFile.getMaxPageWidth()) > getWidth()) {
                float a = currentXOffset + toCurrentScale(pdfFile.getMaxPageWidth());
                Log.d("TAG", "Two" + a + " " + getWidth());
                return true;
            }
        } else {
            if (direction < 0 && currentXOffset < 0) {
                Log.d("TAG", "Three" + currentXOffset);
                return true;
            } else if (direction > 0 && currentXOffset + pdfFile.getDocLen(zoom) > getWidth()) {
                float b = currentXOffset + pdfFile.getDocLen(zoom);
                Log.d("TAG", "Four" + b + " " + getWidth());
                return true;
            }
        }
        Log.d("TAG", " " + direction);
    ...
    HashMap<ImageView, PointLoc> pathHashMap = new HashMap<>();
    ...
    private void adjustCurrentLocation() {
        ...
        if (present) {
            ImageView curIV = (ImageView) getChildAt(pos);

            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(36, 36);
            params.topMargin = (int) ((getScaledHeight() - getScaledHeight() * yFactor) + currentYOffset);
            params.leftMargin = (int) ((getScaledWidth() - getScaledWidth() * xFactor) + currentXOffset);

            curIV.setLayoutParams(params);
        }
    }

    private void adjustCurrentPath() {
        Iterator hmIterator = pathHashMap.entrySet().iterator();
        while (hmIterator.hasNext()) {
            Map.Entry mapElement = (Map.Entry) hmIterator.next();

            PointLoc loc = (PointLoc) mapElement.getValue();

            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(36, 36);
            params.topMargin = (int) ((getScaledHeight() - getScaledHeight() * loc.yFactor) + currentYOffset);
            params.leftMargin = (int) ((getScaledWidth() - getScaledWidth() * loc.xFactor) + currentXOffset);
            ((ImageView) mapElement.getKey()).setLayoutParams(params);
        }
    }


    public void setCurrentLocationChild(double xFactor, double yFactor) {
        ...
        if (present) {
            ...
            ImageView currentLocation = new ImageView(getContext());
            currentLocation.setImageDrawable(getResources().getDrawable(R.drawable.current_path));
            ...
            params.topMargin = (int) ((getScaledHeight() - getScaledHeight() * yFactor) + currentYOffset);
            params.leftMargin = (int) ((getScaledWidth() - getScaledWidth() * xFactor) + currentXOffset);
            addView(currentLocation, params);
            pathHashMap.put(currentLocation, new PointLoc(xFactor, yFactor));
        }
    }
// #CW current working
    public void setCurrentRunningLocation(double xFactor, double yFactor, PDFView pdfView) {
        this.xFactor = xFactor;
        this.yFactor = yFactor;
        ImageView curIV = new ImageView(getContext());
        curIV.setImageDrawable(getResources().getDrawable(R.drawable.current_path));

        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(36, 36);
        params.topMargin = (int) ((getScaledHeight() - getScaledHeight() * yFactor) + currentYOffset);
        params.leftMargin = (int) ((getScaledWidth() - getScaledWidth() * xFactor) + currentXOffset);

        addView(curIV, params);
        pathHashMap.put(curIV, new PointLoc(xFactor, yFactor));
//        pdfView.zoomTo(2f);
        removeView(stopPoint);
        final Handler handler = new Handler();
        handler.postDelayed(() -> removeView(curIV), 1000);


        curIV.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
            @Override
            public void onGlobalLayout() {
                curIV.getViewTreeObserver().removeGlobalOnLayoutListener(this);

                Rect rect = new Rect();
                curIV.getHitRect(rect);
                // Log.e("~~>","pdfScroll ::"+pdfScroll );

                int screenWidth = Resources.getSystem().getDisplayMetrics().widthPixels;
                int screenHeight = Resources.getSystem().getDisplayMetrics().heightPixels;
                Rect screen =new  Rect(0, 0, screenWidth, screenHeight);

                if(screen.intersect(rect.left,rect.top,rect.right,rect.bottom))
                {
                    ...
                    PointF pf =new PointF(curIV.getX(),curIV.getY());
                   // curIV.getViewTreeObserver().removeGlobalOnLayoutListener(this);
                    zoomCenteredRelativeTo(15f,pf);
                    ...
                    /*float s = dragPinchManager.scale;
                    Log.e("??","dragPinchManager scale :: "+ s);
                    Log.e("??","current IV   :: "+ curIV.getX()+":"+curIV.getY());
                    Log.e("??","current IV pivot  :: "+ curIV.getPivotX()+":"+curIV.getPivotY());
                    int pos[] = new int[2];
                    curIV.getLocationOnScreen(pos);
                    int centerX = pos[0] + curIV.getMeasuredWidth() / 2;
                    int centerY = pos[1] + curIV.getMeasuredHeight() / 2;
                    Log.e(">>","centerX  centerY : "+centerX +centerY);
                    curIV.getLocationInWindow(pos);
                    int center_X = pos[0] + curIV.getMeasuredWidth() / 2;
                    int center_Y = pos[1] + curIV.getMeasuredHeight() / 20;
                    Log.e(">>","centerX  centerY : "+center_X +center_Y);
                    //   zoomWithAnimation(CENTER_IN_PARENT,CENTER_IN_PARENT,1.35f);
                    // zoomWithAnimation(CENTER_IN_PARENT, CENTER_IN_PARENT, (float) 1.35);
                    //   PointF pf =new PointF(curIV.getX()+30,curIV.getY()+30);
                    int xFactor = (((pdfView.getWidth() / 2) - rect.left) / rect.width());
                    int yFactor = (((pdfView.getHeight() / 2) - rect.top) / rect.height());
                    Log.e("??","Factor    :: "+ xFactor+":"+yFactor);
                   *//* int xOffset = rect.left;
                    int yOffset = rect.top;*//*

                    int xOffset = rect.right;
                    int yOffset = rect.bottom;*/

                    //  zoomCenteredTo(430,pf);
                    // getHeight()/2
                    // zoomWithAnimation(4500, 1700, (float) 2.00);
                    // zoomWithAnimation(0,0,50);
                    //zoomWithAnimation((float)xFactor,(float) yFactor,50);
                    // pageFillsScreen();
                    // zoomCenteredTo(50,pf);
                    //pdfView.setMinZoom(60);
                    // pdfView.clearAnimation();
                    // PointF pf= new PointF(curIV.getX(),curIV.getY());
                    //  zoomCenteredTo(40,pf);
                    //2.0555243f
                    //animationManager.startZoomAnimation(curIV.getX(), curIV.getY(),100, 5);
                    //getWidth() / 2, getHeight() / 2
                  // pdfView.zoomWithAnimation(1 .35f);
                   //pdfView.setMinZoom(1.35f);
                   //  pdfView.zoomWithAnimation(rect.left,rect.top,7);
                    //2.8218164f
                   /* if (pdfView.getZoom() < pdfView.getMidZoom()) {
                        pdfView.zoomWithAnimation(curIV.getX(), curIV.getY(), pdfView.getMidZoom());
                    } else if (pdfView.getZoom() < pdfView.getMaxZoom()) {
                        pdfView.zoomWithAnimation(curIV.getX(), curIV.getY(), pdfView.getMaxZoom());
                    } else {
                        pdfView.resetZoomWithAnimation();
                    }*/
                }
    ...
    public boolean checkLocationInBounds(Double top,Double right,Double bottom,Double left)
             {
        return curLatitude > left && curLatitude < right && curLongitude < top && curLongitude > bottom;
    }

    public void markedStopLocation(double xFactor, double yFactor) {
        this.xFactor = xFactor;
        this.yFactor = yFactor;
        stopPoint = new ImageView(getContext());
        stopPoint.setImageDrawable(getResources().getDrawable(R.drawable.ic_cur_location));

        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(36, 36);
        params.topMargin = (int) ((getScaledHeight() - getScaledHeight() * yFactor) + currentYOffset);
        params.leftMargin = (int) ((getScaledWidth() - getScaledWidth() * xFactor) + currentXOffset);
        addView(stopPoint, params);
        pathHashMap.put(stopPoint, new PointLoc(xFactor, yFactor));
    }

     public void addPointToPath(double xFactor, double yFactor,boolean isSavedTrack) {
        this.xFactor = xFactor;
        this.yFactor = yFactor;
        ImageView currentLocation = new ImageView(getContext());

        if(isSavedTrack) {
            currentLocation.setImageDrawable(getResources().getDrawable(R.drawable.ic_ellipse));
        } else {
            currentLocation.setImageDrawable(getResources().getDrawable(R.drawable.new_track));
        }
        ...
        params.topMargin = (int)
                ((getScaledHeight() - getScaledHeight() * yFactor) + currentYOffset);
        params.leftMargin = (int) ((getScaledWidth() - getScaledWidth() * xFactor) + currentXOffset);
        addView(currentLocation, params);
        pathHashMap.put(currentLocation, new PointLoc(xFactor, yFactor));
...
//        }, 3000);
 ...
 if (renderedBitmap.isRecycled()) {
    ...
    public void moveTo(float offsetX, float offsetY, boolean moveHandle) {
        Log.d(TAG, "moveTo: Old Offsets " + currentXOffset + " " + currentYOffset);
        Log.d(TAG, "moveTo: Doc Length :" + pdfFile.getDocLen(zoom) + " " + zoom + " : " + pdfFile.getDocLen(zoom) / zoom);
        Log.d(TAG, "moveTo: Swipe :" + swipeVertical);
        /*Log.d(TAG, "moveTo: Size    "+pdfFile.getMaxPageWidth()+" "+pdfFile.getMaxPageHeight());
        Log.d(TAG, "moveTo: Size    "+getWidth()+" "+getHeight());*/
        if (swipeVertical) {
            // Check X offset
            ...
            if (scaledPageWidth < getWidth()) {
                ...
                if (offsetX > getWidth() / 2) {
                    offsetX = getWidth() / 2;
                } else if (offsetX + scaledPageWidth < getWidth() / 2) {
                    offsetX = getWidth() / 2 - scaledPageWidth;
                }
            }
            /*if (offsetX > 0) {
                offsetX = 0;
            ...
            if (contentHeight < getHeight()) { // whole document height visible on screen
                ...
                if (offsetY > getHeight() / 2) { // top visible
                    offsetY = getHeight() / 2;
                } else if (offsetY + contentHeight < getHeight() / 2) { // bottom visible
                    offsetY = -contentHeight + getHeight() / 2;
                }
            }
            ...
            Log.d(TAG, "moveTo: Scale :" + scaledPageWidth + " " + contentHeight);
        } else {
            // Check Y offset
            ...
            Log.d(TAG, "moveTo: Scale :" + scaledPageHeight + " " + contentWidth);
        }
        ...
        //Log.d(TAG, "moveTo: Offsets "+offsetX+" "+offsetY);
        Log.d(TAG, "moveTo: Cur Offsets " + currentXOffset + " " + currentYOffset);
        if (onMoveListener != null) {
            onMoveListener.onMove(new RectF(currentXOffset, currentYOffset, currentXOffset + toCurrentScale(pdfFile.getMaxPageWidth()), currentYOffset + toCurrentScale(pdfFile.getMaxPageHeight())));
        }
        redraw();
    }

    public float getScaledWidth() {
        return toCurrentScale(pdfFile.getMaxPageWidth());
    }

    //    public float getScaledHeight(){
//        return toCurrentScale(pdfFile.getMaxPageHeight());
//    }
    public float getScaledHeight() {
        Float size = 0.0f;
        if (pdfFile.getMaxPageHeight() != 0.0f) {
            size = pdfFile.getMaxPageHeight();
        }
        return toCurrentScale(size);
    }
    ...
    public class Configurator {
        ...
        public Configurator setonMoveListener(OnMoveListener onMoveListener) {
        ...
        public void load() {
            ...
            PDFView.this.setOnMoveListener(onMoveListener);
            if (pageNumbers != null) {
    ...
    private void setOnMoveListener(OnMoveListener onMoveListener) {
        this.onMoveListener = onMoveListener;
    }
}
